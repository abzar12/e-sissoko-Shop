...
export function CustomerPassword() {
    ...
    const handleEmail = async (e) => {
        ...
        try {
            const resp = await fetch(`${import.meta.env.VITE_API_URL}/customers/forgotPassword/${inputemail}`,{
                method: "POST",
    ...
    const handleClose = () => {
        setShow(false)
        if(status) navigate(`/customer/verify-OTP?email=${Email}`)
    }
    return (
        <>
            ...
            <div className="w-full">
                <div className="sm:mx-auto mx-3 mt-[35vh]  bg-[var(--bg-color-primary)] min-w-[100px] max-w-[400px] shadow-[1px_1px_15px_var(--card-shadow)] p-3 text-center rounded-md text-[var(--text-color-secondary)]">
                    ...
                    <p className="opacity-50 mt-3" >Please type your email to get and OTP</p>
                    <form action="" onSubmit={handleEmail} className="px-5">
                        <div className="w-full border px-3 py-1 sm:py-2 rounded-md my-3">
                            <input type="email" placeholder="example@gmail.com"
                                className=" outline-none focus:outline-none bg-transparent w-full " name="email" required />
                        </div>
