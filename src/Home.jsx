...
function Home() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await fetch(`http://localhost:5330/api/`);
                if (!resp.ok) {
        ...
        <>
            ...
            <main className="grid grid-cols-[1fr,4fr] ">
                ...
                <section className="px-5 pt-10 h-[90vh] overflow-y-scroll">
                    <div className="ac_Items flex flex-wrap justify-center gap-3 py-5 ">
                        {
                            products.map( (P, index) => (
                                <div key={index} className="item w-[180px] min-h-[220px] my-3 text-left  border-2">
                                    <img src={`http://localhost:5330/public/img/${P.Image}`} alt={P.Image} className="w-full h-[100px]" />
                                    <div className="ac_textItem p-3">
